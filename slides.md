...
<!--
大家上午好，今天我会分享一些我对自动化测试的理解，过程中大家有什么疑问，或者有更好的实践方法，也可以随时提出来互相交流。
-->
...
<!--
好的，接下来咱们就正式进入今天的分享内容。为了让大家对整体脉络更清晰，我先简单说一下今天的分享框架：

首先，我会先从基础概念入手，和大家聊聊什么是自动化测试，以及它在咱们实际工作中常见的分类。

接着，咱们会聚焦到实际落地层面，重点分析自动化测试的成本构成，以及它能给咱们团队带来的核心收益，帮大家更全面地判断 “哪些场景值得引入自动化测试”；

然后，我会结合咱们团队的业务场景，对刚才提到的各类自动化测试进行具体举例讲解；

最后我会对今天的内容做个总结。

好的，现在我们开始分享的第一部分——什么是自动化测试及常见分类。
-->
...
<!--
就是用自动化工具来执行测试用例，自动比对实际结果和我们预期的结果是否一致，最后还能自动生成测试报告的整个过程。它能够减少人工的干预，提高测试效率和准确性。

举个咱们工作里的例子：比如测试用户注册功能时，手动测试需要一次次输入不同手机号、密码，提交后还要自己核对提示信息；而自动化测试就是用工具写好脚本，让它自动完成输入、提交、校验结果这一系列操作，最后还能把哪些用例通过、哪些失败整理成报告供我们查看测试结果。
-->
...
首先是离我们开发最近的单元测试，它是对最小可测试单元，通常是我们写的函数或者方法进行测试，来验证单个组件的功能是否正常，各种语言基本都有自己的单元测试框架，JS里常用的就是Jest以及Vitest，我们后面会用Jest来讲解单元测试和集成测试。

接着是集成测试，相较于单元测试测试单个组件是否正常工作，集成测试关心的是多个组件共同工作的时候会不会出问题，就比如前后端自己都测都没问题，但是对接就有可能会出问题。基本单元测试框架也都会支持集成测试。
...
左侧是由麦克·科恩(Mike Cohn)提出的一个经典的测试金字塔模型，从上到下分为三层，每一层代表一类测试，且层级越低，测试数量越多、成本越低、执行速度越快；层级越高，测试数量越少、成本越高、维护难度越大，测试用例比例从上到下是1：2：7，但是在实际应用中没有必要按严格照这个标准做，因为我们实际上关注的就是我们的系统能否按照预期进行工作,只要我们的测试代码可以完成这个目标即可，
那么我们对哪些测试对象编写测试代码能够高效率的完成这个目标呢？讲解测试对象
...
分别是测试工具的学习成本，
开发人员测试意识的培养，例如之前想测注册，但是验证码没有办法拿到所以测不了，因此在编写代码的时候就要想到如何让代码变得可测试，
...
好的，目前我们已经对自动化测试基本概念进行了扫盲，接下来会结合具体业务场景对前面提到的各类自动化测试进行具体的举例讲解，大家对前面讲的内容有什么疑问吗？
没有的话，我们接着向下进行。
-->
...
Jest具有这些特色功能：
- 极简配置：默认即可运行测试，当然我们通常也会通过配置文件进行自定义配置。
- 内置断言库：提供丰富、可读性高的断言函数（`Jest Matchers`），支持链式调用。
- 内置测试运行器：支持按约定自动识别测试文件，并行执行测试、监视模式，可自动识别变更并仅运行相关测试。
- Mocking功能：可以模拟函数、模块、定时器、第三方库或 API 调用。用于构建稳定的测试环境，防止测试代码受外部依赖的干扰。
...
因此我这里以我最近写的一个功能举例，它是一个可以从字符串中提取固定格式子串并检查其是否在变量列表中的函数
-->
...
需要注意的是，快照测试只是测试工具提供的一种测试手段，他不是一种单独的测试类型。
...
<!--
例如我们要对这样一个功能编写单元测试：它可以解析prompt（JSON）并将其转换为渲染时需要的数据结构，我们给它起个名字叫做parsePrompt。
...
因为导出服务会用到浏览器，所以这里我们将超时时长设置为60s，并在测试执行前初始化浏览器池
（按空格翻页）
在执行后解放浏览器池释放资源。
-->
...
<!--
接下来开始正式编写测试脚本，编写的逻辑与单元测试差不多，都是先准备测试数据，执行测试代码，断言运行结果。
-->
...
<!--
接下来介绍一下前端特有的一类测试，组件测试。主要测试前端开发中的UI组件。
-->
...
需要特殊说明的是，组件测试不能看到实际渲染的样式，只能拿到DOM结构
...
<!--
端到端测试与前面两类测试不同的地方在于他是在真实用户环境中进行的测试

以Playwright以及基于它的测试框架@playwright/test为工具来介绍端到端测试在项目中的实践。
-->
...
@playwright/test支持一些特性，包括右侧这些，下面我们会详细介绍。
-->
...
<!--
这里我们以网站登录为例，来讲解如何编写一个端到端测试用例，首先跳转到要测试的网页，接下来执行用户操作，最后断言操作结果。
-->
...
<!--
接下来我们执行命令运行测试。测试结果如图所示：

可以看到运行测试的数量和使用的worker线程数以及执行用时
-->
...
<!--
需要注意的是只支持@playwright/test最新适配的浏览器内核版本，不支持旧版浏览器
-->
...
<!--
那么我们如何实现多浏览器运行测试，只需要在配置文件中，多个测试项目及使用的设备
-->
...
<!--
我们可以通过抓包工具抓取请求查看UA，可以看到确实使用了不同的UA来运行测试
-->
...
<!--
除了Playwright还有一些其他的测试工具，例如Cypress和Selenium，这里对这些方案在一些需要关心的维度上进行了对比，包括是否支持多浏览器，旧浏览器，多窗口，性能如何，是否支持并行测试，社区大小，学习成本。
-->
...
<!--
到这里，今天的分享内容就基本梳理完了，咱们最后做个整体总结。
首先，咱们明确了自动化测试的定义与分类：它是通过工具执行用例、比对结果、生成报告的过程。按照测试粒度，我们有单元测试，集成测试和端到端测试三种分类。
其次，关于成本与收益的权衡：前期需要投入工具的学习成本，开发人员的测试意识的培养、测试脚本开发的时间与精力，后期还要考虑脚本维护；但长期来看，在测试效率提升、减少人为失误等方面能带来显著收益。
最后，咱们结合实际业务场景对三种测试进行了实践。
以上内容覆盖了自动化测试的基本概念和实际落地，希望能为大家能够有所收获。
-->
